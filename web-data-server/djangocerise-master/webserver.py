...
class Runner(object):
    ...
    def main(self):
        ...
        try:
            log.info("Start the server on %s:%s" % (options.IP_ADDRESS,
                                                    str(options.PORT)))
            self.server.start()
...
sys.path? Does it have syntax errors?): %s" % (cmd_options.config, e))
...
if __name__ == "__main__":
    ...
    if options.RUN_AS_DAEMON:
        ...
        if os.path.exists(options.PIDFILE):
            log.info(
                "PID file exists: %s, webserver already running, exiting" % (
                    options.PIDFILE))
            sys.exit(1)
        try:
            pid = os.fork()
